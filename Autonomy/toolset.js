...
//	    				show: false,
        ...
        this._handler.setInputAction(function (movement) {
            ...
            if (m_w < 200) {
                ...
                var h = h_w - h_n;
                document.getElementById("cfglobe_toolset").style.bottom = (h + 82) + "px";
...
//			m_toolset_list.appendChild(m_toolset_icon);
    ...
    Toolset.prototype._addressClick = function () {
        ...
        $.getJSON("./static/city_poi.json", function (data) {
            if (data.features) {
                self._clearAddressList();
                var addressList = [];
                for (let i = 0; i < data.features.length; i++) {
                    var city_name = data.features[i].properties.NAME;
                    var sub_address = city_name.substring(0, city_name.length - 1);
                    if (city_name == m_address || m_address == sub_address) {
...
//	m_select.appendChild(m_toolset_top_button_clear);
    ...
    Toolset.prototype._loadAnalysis = function () {
        ...
        m_toolset_top_analysis_button2.onclick = function () {
            ...
            var options = {
                lon: m_lon, lat: m_lat, alt: m_alt, horizontal: m_horizontal, pitch: m_pitch, dist: m_dist,
...
//	m_info.appendChild(m_toolset_top_path_button4);
    ...
    Toolset.prototype._loadShp = function () {
        ...
        m_toolset_top_shp_button1.onclick = function () {
            ...
            self._globe.shpParser.add("cfglobe_toolset_top_shp_file", {
                callback: self._shpCallback,
                lineWidth: lineWidth,
                color: m_color,
...
//			obj.setAttribute("class", "cfglobe-toolset-btn-open");
            ...
            /*高清出图配置参数 by chenglei */
            var canvas = self._globe._viewer.scene.canvas;
            var base64url = canvas.toDataURL('image/png');
            var binStr = atob(base64url.split(',')[1]),
                len = binStr.length,
                arr = new Uint8Array(len);
            for (var i = 0; i < len; i++) {
                arr[i] = binStr.charCodeAt(i);
            }
            var triggerDownload = document.getElementById('loadimg');
            triggerDownload.download = '0.png';
...
//		self._globe.viewshed.clear();
            ...
            if (sceneMode == Cesium.SceneMode.SCENE3D) {
               var imageLevelDiv= document.getElementById("imageLevel");
               if(imageLevelDiv){
                   imageLevelDiv.style.display="none";
               }
...
//		    obj.parentNode.firstChild.setAttribute("class", "cfglobe-toolset-btn-3d");
                ...
                var imageLevelDiv= document.getElementById("imageLevel");
                if(imageLevelDiv){
                    imageLevelDiv.style.display="block";
                }
    ...
    Toolset.prototype._rotateByPointList = function (pointList, i) {
        ...
        if (self._rotatePoint) {
            ...
            var options = {
                ...
                lat: pointList[i].lat,
                alt: pointList[i].alt,
                heading: pointList[i].heading,
                pitch: pointList[i].pitch,
                roll: 0
            };
            self._globe.setView(options);
            setTimeout(function () {
