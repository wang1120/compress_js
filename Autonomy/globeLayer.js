...
define([], function () {
    ...
    /*
       * 2019/11/6 0006
       * @author wangchenglei
       * @method  addSupermapScene
       * @for {Object}
       * @param {Object}
       * @return {Object}
       */
    GlobeLayer.prototype.addSupermapScene = function (options) {
        var self = this;
        ...
        try {
            var promise = m_scene.open(m_url);
            Cesium.when(promise, function (layers) {
                for (let i = 0; i < layers.length; i++) {
                    var layer = layers[i];
                    self._layerList[self._layerList.length] = layer;
                    var startTime = new Date().getTime() + "a" + parseInt(100 * Math.random());
                    self._globe.layerManager._add("vectorLayer", startTime, layer.name, "", m_show);
                }
            })
            return promise;
        } catch (e) {
            var title = '加载工作空间失败，请检查网络连接状态或者url地址是否正确？';
            this._widget.showErrorPanel(title, undefined, e);
        }
    }
    /*
     * 2019/11/6 0006
     * @author wangchenglei
     * @method  addS3MTilesLayer
     * @for {Object}
     * @param {Object}
     * @return {Object}
     */
    GlobeLayer.prototype.addS3MTilesLayer = function (options) {
        var self = this;
        var m_show = true;
        if (options && typeof options.show === 'boolean') {
            m_show = options.show;
        ...
        var m_cullEnabld = options.cullEnabled ? options.cullEnabled : true;
        var m_index = options.index ? options.index : "";
        var m_scene = self._viewer.scene;

        try {
            var promise = m_scene.addS3MTilesLayerByScp(m_url, {name: m_name, cullEnabled: m_cullEnabld}, m_index);
            promise.then(function (layer) {
                ...
                self._layerList[self._layerList.length] = layer;
                var startTime = new Date().getTime() + "a" + parseInt(100 * Math.random());
                self._globe.layerManager._add("vectorLayer", startTime, l_name, "", m_show);
            });
            return promise;
        } catch (e) {
            var title = '加载url失败，请检查网络连接状态或者url地址是否正确？';
            this._widget.showErrorPanel(title, undefined, e);
        }
     ...
     * @return {Object}
     */
    GlobeLayer.prototype.addSupermapLayer = function (options) {
        var self = this;
        var m_show = true;
        if (options && typeof options.show === 'boolean') {
            m_show = options.show;
        ...
        var m_index = options.index ? options.index : self._viewer.imageryLayers.length;
        try {
            var layerProvider = new Cesium.SuperMapImageryProvider({
                url: m_url,
                name: m_name,
            ...
            var layer = self._viewer.imageryLayers.addImageryProvider(layerProvider, m_index);
            var l_name;
            if (m_name === '') {
                l_name = layer.name;
            } else {
    ...
    * @return {Object}
    */
    GlobeLayer.prototype.addUrlTileLayer = function (options) {
        var self = this;
        try {
            var m_show = true;
            if (options && typeof options.show === 'boolean') {
...
//	}
    ...
    */
    GlobeLayer.prototype.loadGoogleMap = function (options) {
        ...
        var m_minimumLevel = options.minimumLevel ? options.minimumLevel : 0;
        var m_maximumLevel = options.maximumLevel ? options.maximumLevel : 18;
        var m_west = options.west ? options.west : -180;
        var m_south = options.south ? options.south : -90;
        var m_east = options.east ? options.east : 180;
        var m_north = options.north ? options.north : 90;
...
//  });
    ...
    */
     ...
     * @returns {*}
     */
    GlobeLayer.prototype.addLocalImageTiles = function (options) {
        var self = this;
        ...
        var m_north = options.north ? options.north : 90;
        var layers = self._viewer.imageryLayers;
        var imagery = new Cesium.TileMapServiceImageryProvider({
            name: m_name,
            url: m_url,
            // tilingScheme: new Cesium.WebMercatorTilingScheme(),
            fileExtension: 'png',
        ...
        var tilesMapLayer = layers.addImageryProvider(imagery);
        var startTime = new Date().getTime() + "a" + parseInt(100 * Math.random());
        tilesMapLayer.lid = startTime;
        self._layerList[self._layerList.length] = tilesMapLayer;
        return tilesMapLayer;
    }
